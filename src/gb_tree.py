...
try:
    ...
    loss_param='%%loss%%'
    learningRate_param=float('%%learning_rate%%')
    numIterations_param=int('%%num_iterations%%')
    ...
    predictors = ["Drug","BP", "Na", "K", "Age"]
    target = "Age"
    maxDepth_param=5
    maxBins_param=20
    modelpath_base = "/tmp/model1234"
    model_type="regression"
    loss_param="leastSquaresError"
    learningRate_param=0.1
    numIterations_param=10
    import shutil
    try:
